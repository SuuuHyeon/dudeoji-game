...
export function useGame(auth, antiCheat) {
  ...
  const scheduleSpawn = (customDelay = null) => {
    ...
    spawnTimer = setTimeout(() => {
      ...
      const nextDelay =
        type === 'bomb' ? getSpawnDelay() * BOMB_NEXT_SPAWN_RATIO : null;
  ...
  const handleHit = (index, entity) => {
    ...
    if (blocked) {
      antiCheat.enforceAntiCheat(abortGameWithLog);
      return;
    }
  ...
  const endGame = async () => {
    ...
    let flagReason = antiCheat.getFlagReason();
    if (!flagReason && score.value > SCORE_HARD_CAP)
      flagReason = 'score_hard_cap';
    if (!flagReason && score.value < SCORE_LOWER_CAP)
      flagReason = 'score_lower_cap';
    ...
    if (currentSessionId && auth.currentName.value) {
      submitGameLog(
        ...
        isFlagged,
        flagReason,
        antiCheat.getTelemetry(),
      ).catch(() => {}); // 전송 실패해도 게임 진행에 영향 없도록 에러 삼킴
    }
  ...
  // 안티치트 적발로 강제 종료될 때 로그를 남기고 종료하는 함수
  const abortGameWithLog = () => {
    if (currentSessionId && auth.currentName.value) {
      submitGameLog(
        currentSessionId,
        auth.currentName.value,
